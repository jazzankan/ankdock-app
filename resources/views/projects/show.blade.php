                ...
                @if(count($belongingfiles) > 0)
                    <ul class="mt-4">
                        ...
                        @foreach($belongingfiles as $f)
                            <li class="list-inline-item"><a class="text-blue-700 hover:underline"
                                                            href="https://ank.webbsallad.se/storage/files/{{ $f->filename }}"
                ...
                @if(count($projcomments) > 0)
                    ...
                    @foreach($projcomments as $c)
                        @if($loop->iteration > 2)
                            @break
                        @endif
                        <p><span class="text-yellow-800">{{ $c->body }}</span><br><span
                                class="text-xs">{{ $c->created_at }}</span><br><i><b>{{ $c->user->name }}</b></i><br>.....
                        </p>
                    @endforeach
                @endif
                <div x-data="{ isOpen: false }">
                    @if(count($projcomments) > 2)
                        <a x-on:click="isOpen = !isOpen" class="text-blue-600 hover:underline" href="#comments">Tidigare
                            kommentarer:</a>
                    @endif
                    @foreach($projcomments as $c)
                        @if($loop->iteration > 2)
                            <div x-show="isOpen"><span class="text-yellow-800">{{ $c->body }}</span><br><span
                                    class="text-xs">{{ $c->created_at }}</span><br><i><b>{{ $c->user->name }}</b></i><br>.....
                            </div>
                        @endif
                    @endforeach
                                            ...
                                            class="md:float-right mr-4"> Deadline: <span
                                                ...
                                                class=""><b>{{$todo->assigned}}</b></span>&nbsp<span>@if($todo->details)
                                                    <a x-on:click="isOpen = !isOpen"
                                                       ...
                                                       href="#todos">Detaljer</a>@endif</span></span><br>
                                        <div x-show="isOpen"
                                             ...
                                             style='white-space:pre-wrap'>{{ $todo->details }}</div>
                                    </li>
                                @endif
                            @endforeach
                        </ul>
